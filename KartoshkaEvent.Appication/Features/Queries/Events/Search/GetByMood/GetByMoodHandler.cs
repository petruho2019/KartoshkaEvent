...
{
    public class GetByMoodHandler(
        IKartoshkaEventContext context) : IRequestHandler<GetByMoodQuery, List<EventGetByMood>>
    {
        public async Task<List<EventGetByMood>> Handle(GetByMoodQuery request, CancellationToken ct)
        {
            return await context
               ...
               .Where(e => e.LocationsOfEvents.Any(t => t.Mood == request.Mood))
               .Select(e => new EventGetByMood
               {
                   Id = e.Id,
                   Subject = e.Subject,
                   ...
                   Locations = e.LocationsOfEvents.Where(l => l.ModerationStatus.Equals(ModerationStatus.Approved))
                       .Select(ae => new LocationGetByMood()
                       {
                           AddressId = ae.Id,
                           City = ae.City,
                           Street = ae.Street,
